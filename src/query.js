...
const optionalPendingZero = require('./optionalPendingZero')
...
function parts (date, options = {}) {
  ...
  if (!options.op || options.op === '=') {
    if (date.match(/^\d{4}-\d{2}-\d{2}$/)) {
      possibilities.push(optionalPendingZero(date))

      if (!options.strict) {
        let p
        // p - part of the month
        p = earlyMidLate[Math.floor(date.substr(8, 2) / 31 * 3)]
        possibilities.push('(|' + p + ' )' + optionalPendingZero(date.substr(0, 7)))
        // p - part of the year
        p = earlyMidLate[Math.floor((date.substr(5, 2) - 1) / 12 * 3)]
        possibilities.push('(|' + p + ' )' + optionalPendingZero(date.substr(0, 4)))

        p = earlyMidLate[Math.floor((date.substr(5, 2) - 1) / 12 * 3)]
        possibilities.push('(|' + p + ' )' + optionalPendingZero(date.substr(0, 4)))

        possibilities.push(optionalPendingZero(date.substr(0, 3) + '0s'))

        p = earlyMidLate[Math.floor((date.substr(3, 1) / 10 + (date.substr(5, 2) - 1) / 120) * 3)]
        possibilities.push('(|' + p + ' )' + optionalPendingZero(date.substr(0, 3)) + '0s')

        const cent = parseInt(date.substr(0, 2)) + 1
        p = earlyMidLate[Math.floor((date.substr(2, 2) / 100 + (date.substr(5, 2) - 1) / 1200) * 3)]
        possibilities.push('(|' + p + ' )[Cc]' + optionalPendingZero(('' + cent).padStart('0', 2)))
      }
    }
    if (date.match(/^\d{4}-\d{2}$/)) {
      possibilities.push('(|early |mid |late )' + optionalPendingZero(date + '(-[0-9]{2})?'))
    ...
    if (date.match(/^\d{4}$/)) {
      possibilities.push('(|early |mid |late )' + optionalPendingZero(date + '(-[0-9]{2}(-[0-9]{2})?)?'))
      ...
      if (!options.strict) {
        ...
        possibilities.push('(|' + p.join(' |') + ' )' + optionalPendingZero(date.substr(0, 3)) + '0s')
        ...
        const cent = parseInt(date.substr(0, 2)) + 1
        possibilities.push('(|' + p.join(' |') + ' )[Cc]' + optionalPendingZero(('' + cent).padStart('0', 2)))
      }
    } else if (date.match(/^\d{4}s$/)) {
      possibilities.push('(|early |mid |late )' + optionalPendingZero(date.substr(0, 3)) + '(0s|[0-9](-[0-9]{2}(-[0-9]{2})?)?)')
      ...
      possibilities.push('(|early |mid |late )[Cc]' + optionalPendingZero(('' + cent).padStart('0', 2)))
      possibilities.push('(|early |mid |late )' + optionalPendingZero(year.substr(0, 2)) + '[0-9](0s|[0-9](-[0-9]{2}(-[0-9]{2})?)?)')
    }
    ...
    if (date.match(/^[Cc]\d+$/)) {
      ...
      possibilities.push('(|early |mid |late )' + optionalPendingZero(year.substr(0, 2)) + '[0-9](0s|[0-9](-[0-9]{2}(-[0-9]{2})?)?)')
      possibilities.push('(|early |mid |late )[Cc]' + optionalPendingZero(cent))
    }
  } else if (options.op === '<') {
    if (date.match(/^\d{4}-\d{2}-\d{2}$/)) {
      ...
      if (date.substr(8, 2) !== '01') {
        possibilities.push(optionalPendingZero(date.substr(0, 8) + regexpRangeDouble(1, date.substr(8, 2) - 1)))
      }

      if (!options.strict) {
        possibilities.push(optionalPendingZero(date.substr(0, 7)))
      }
    }
    if (date.match(/^\d{4}-\d{2}$/)) {
      ...
      if (date.substr(5, 2) !== '01') {
        possibilities.push(optionalPendingZero(date.substr(0, 5)) + regexpRangeDouble(1, date.substr(5, 2) - 1) + '(-[0-9]{2})?')
      }

      if (!options.strict) {
        possibilities.push(optionalPendingZero(date.substr(0, 4)))
      }
    }
    if (date.match(/^\d{4}$/)) {
      ...
      if (date[3] !== '0') {
        possibilities.push('(|early |mid |late )' + optionalPendingZero(date.substr(0, 3)) + regexpRange(0, date[3] - 1) + '(-[0-9]{2}(-[0-9]{2})?)?')
      }

      if (!options.strict) {
        possibilities.push('(|early |mid |late )' + optionalPendingZero(date.substr(0, 3)) + '0s')
      }
    } else if (date.match(/^\d{4}s$/)) {
      const cent = parseInt(date.substr(0, 2)) + 1
      ...
      for (let i = 0; i < 2; i++) {
        if (cent[i] > 0) {
          possibilities.push('(|early |mid |late )[Cc]' + optionalPendingZero(cent.substr(0, i) + '0') + '[0-9]'.repeat(1 - i))
        }
        if (cent[i] > 1) {
          possibilities.push('(|early |mid |late )[Cc]' + optionalPendingZero(cent.substr(0, i)) + regexpRange(1, cent[i] - 1) + '[0-9]'.repeat(1 - i))
        }

        if (year[i] > 0) {
          possibilities.push('(|early |mid |late )' + optionalPendingZero(year.substr(0, i) + '0') + '[0-9]'.repeat(2 - i) + '(0s|[0-9](-[0-9]{2}(-[0-9]{2})?)?)')
        }
        if (year[i] > 1) {
          possibilities.push('(|early |mid |late )' + optionalPendingZero(year.substr(0, i)) + regexpRange(1, year.substr(i, 1) - 1) + '[0-9]'.repeat(2 - i) + '(0s|[0-9](-[0-9]{2}(-[0-9]{2})?)?)')
        }
      }
    }
  } else if (options.op === '>') {
    if (date.match(/^\d{4}-\d{2}-\d{2}$/)) {
      ...
      if (date.substr(8, 2) !== '31') {
        possibilities.push(optionalPendingZero(date.substr(0, 8) + regexpRangeDouble(parseInt(date.substr(8, 2)) + 1, 31)))
      }
      ...
      if (!options.strict) {
        ...
        if (date.substr(5, 5) !== '12-31') {
          possibilities.push('(|late )' + optionalPendingZero(date.substr(0, 4)))
        }
        if (date.substr(3, 7) !== '9-12-31') {
          possibilities.push('(|late )' + optionalPendingZero(date.substr(0, 3)) + '0s')
        }
      }
    }
    if (date.match(/^\d{4}-\d{2}$/)) {
      ...
      if (date.substr(5, 2) !== '12') {
        possibilities.push(optionalPendingZero(date.substr(0, 5)) + regexpRangeDouble(parseInt(date.substr(5, 2)) + 1, 12) + '(-[0-9]{2})?')
      }
    ...
    if (date.match(/^\d{4}$/)) {
      ...
      if (date[3] !== '9') {
        possibilities.push('(|early |mid |late )' + optionalPendingZero(date.substr(0, 3)) + regexpRange(parseInt(date[3]) + 1, 9) + '(-[0-9]{2}(-[0-9]{2})?)?')
      }
      ...
      for (let i = 0; i < 2; i++) {
        if (cent[i] < (i === 0 ? maxCent[i] : 9)) {
          possibilities.push('(|early |mid |late )[Cc]' + optionalPendingZero(cent.substr(0, i)) + regexpRange(parseInt(cent[i]) + 1, i === 0 ? maxCent[i] : 9) + '[0-9]'.repeat(1 - i))
        }

        if (year[i] < (i === 0 ? maxYear[i] : 9)) {
          possibilities.push('(|early |mid |late )' + optionalPendingZero(year.substr(0, i)) + regexpRange(parseInt(year.substr(i, 1)) + 1, i === 0 ? maxYear[i] : 9) + '[0-9]'.repeat(2 - i) + '(0s|[0-9](-[0-9]{2}(-[0-9]{2})?)?)')
        }
      }
    }
  }
